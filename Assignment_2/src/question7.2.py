...
def apply_bilateral_filter(img,diameter, sigma_i, sigma_s):
    ...
    for x in range(0,row):
        for y in range(0,col):
            ...
            g_i_1 = 1 - gaussian(img[n_x, n_y,0] - img[x, y,0], sigma_i)
            g_i_2 = 1 - gaussian(img[n_x, n_y,1] - img[x, y,1], sigma_i)
            g_i_3 = 1 - gaussian(img[n_x, n_y,2] - img[x, y,2], sigma_i)
...
img = cv.imread('D:/DIP/Assignment_2/a2_2019701006/input_data/bell.jpg')
result = apply_bilateral_filter(img,5, 24.0, 28.0)
cv.imshow("Original Image",img)
cv.imshow("Inverse Bilateral",result)
cv.waitKey(0)
